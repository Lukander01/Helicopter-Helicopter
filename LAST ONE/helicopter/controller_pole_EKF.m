...
% negative alpha - up
if alpha_ref < 0
p_new = [1.01*p_real(1:2)+j*1.01*imag(p(1:2)) ; 1.01*p(3)];

% positive alpha - down
else
%p_new = [8*p_real(1:2)+j*4*imag(p(1:2)) ; 2*p(3)];
p_new = [1.02*p_real(1:2)+j*1.01*imag(p(1:2)) ; 1.05*p(3)];
...
% calc U with saturation

% A - BK

%  g = tf(ss(A_lin,B_lin,C,[]));
%  s = tf('s');
%  gp = g*(s+1)/s;
% minssss =minreal(ss(gp));
...
%%
    ...
    hold on;
    subplot(2,1,1)
    plot(differenceEKF.Time,differenceEKF(1).Data(:, 1));
    grid on;
    title('Observer error')
    xlabel('t[s]')
    ylabel('\alpha [rad]')
    subplot(2,1,2)
    plot(differenceEKF.Time,differenceEKF(1).Data(:, 2));
    grid on;
    %title('Observer error')
    xlabel('t[s]')
    ylabel('\omega [rad/s]')
...
%%
    figure;
    subplot(3,1,1)
    plot(x_hat.Time,x_hat(1).Data(:, 1))
    hold all
    ...
    xlabel('t[s]')
    ylabel('\alpha [rad]')
    legend('Observer', 'Real',Location='best')
    subplot(3,1,2)
    plot(x_hat.Time,x_hat(1).Data(:, 2))
    hold on
    ...
    subplot(3,1,3)
    plot(x_hat.Time,x_hat(1).Data(:, 3))
    hold all
