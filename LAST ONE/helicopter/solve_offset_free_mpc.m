...
%     x_bounds = [-1, 1; -100, 100; -360, 360];
%     u_bounds = [-1, 1];
% 
%     umin = repmat(u_bounds(:,1), N, 1);
%     umax = repmat(u_bounds(:,2), N, 1);
% 
%     xmin = repmat(x_bounds(:,1), N, 1);
%     xmax = repmat(x_bounds(:,2), N, 1);
%     Phi_x0 = T * x0;  % Reused term
%     Aineq_x = [S; -S];
%     bineq_x = [xmax - Phi_x0; -(xmin - Phi_x0)];
    ...
    if exitflag ~= 1
        ...
        x_bar = reshape(T * x0 + S * u_opt', n_x, N)';
    end
end
