...
% negative alpha - up
if alpha_ref < 0
p_new = [1.01*p_real(1:2)+j*1.01*imag(p(1:2)) ; 1.01*p(3); 0];

% positive alpha - down
else
p_new = [1.1*p_real(1:2)+j*1.01*imag(p(1:2)) ; 1.015*p(3); 0];
end

K = place(minssss.A, minssss.B, p_new);
...
sim("helicopter_Kalman_4dim.slx")
...
%%
    ...
    hold on;
    subplot(2,1,1)
    plot(differenceKalman(1).Data(1, :));
    grid on;
    title('Observer error KALMAN')
    xlabel('t[s]')
    ylabel('\alpha [rad]')
    subplot(2,1,2)
    plot(differenceKalman(1).Data(2, :));
    grid on;
    title('Observer error KALMAN')
    xlabel('t[s]')
    ylabel('\omega [rad/s]')
